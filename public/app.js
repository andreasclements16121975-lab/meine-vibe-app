...
function renderNextEvent() {
  ...
  if (daysEl) daysEl.textContent = diffDays === 0 ? 'Heute' : diffDays === 1 ? 'Morgen' : diffDays;
  ...
  if (detailsEl) {
    ...
    detailsEl.innerHTML = `<div class="flex gap-3 lg:gap-6 mt-3 pt-3" style="justify-content:center;">${cols.map(c => `<div class="min-w-0"><div class="text-white/40 text-[9px] font-bold uppercase tracking-widest mb-0.5">${c.label}</div><div class="text-white text-xs font-semibold truncate">${c.value}</div></div>`).join('')}</div>`;
  }
...
function renderCalendar() {
  ...
  if (firstDayMo > 0) {
    ...
    kwCellFirst.style.cssText = 'text-align:center;font-family:"JetBrains Mono",monospace;font-size:13px;font-weight:700;color:#C83E3E;padding:2px 0;display:flex;align-items:center;justify-content:center;';
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    if (week !== currentWeek) {
      ...
      kwCell.style.cssText = 'text-align:center;font-family:"JetBrains Mono",monospace;font-size:13px;font-weight:700;color:#C83E3E;padding:2px 0;display:flex;align-items:center;justify-content:center;';
  ...
  <div class="flex flex-col gap-3">
    ...
    <style>
        ...
        @media (min-width: 1024px) {
              .fm-chip{font-size:12px;padding:6px 12px;gap:6px;}
              .fm-chip-num{font-size:11px;}
            }
